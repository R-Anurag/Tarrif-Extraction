...
class FederalRegisterPipeline:
    def __init__(self):
        self.connector = FederalRegisterConnector()
    ...
    def run(self, days_back: int = 1):
        print(f"Fetching Federal Register documents from last {days_back} days...")
        
        try:
            documents = self.connector.fetch_documents(days_back=days_back)
            print(f"Found {len(documents)} documents")
            
            # Pre-load existing hashes for fast duplicate checking
            print("Loading existing hashes for deduplication...")
            existing_hashes = self.db.get_existing_hashes(source='federal_register')
    ...
    def process_document(self, doc: Dict, existing_hashes: set = None):
        ...
        print(f"\nProcessing: {doc_number} - {doc.get('title', 'No title')[:60]}...")
        ...
        try:
            ...
            html_path = None
            if doc.get('html_url'):
                html_content = self.connector.download_html(doc['html_url'])
                html_path = self.storage.save_html(html_content, 'federal_register', doc_number)
                html_data = self.html_parser.parse(html_content)
            ...
            if doc.get('pdf_url'):
                try:
                    pdf_content = self.connector.download_pdf(doc['pdf_url'])
                    pdf_path = self.storage.save_pdf(pdf_content, 'federal_register', doc_number)
                    pdf_data = self.pdf_parser.extract_from_bytes(pdf_content)
                    parsed_doc['pdf_hts_codes'] = pdf_data.get('hts_codes', [])
                except Exception as e:
            ...
            # Fast duplicate check using pre-loaded hashes
            ...
            tariff_event = {
                ...
                "source_priority": SOURCE_PRIORITIES["federal_register"],
    ...
    def _build_relationships(self, event_id: str, doc_data: Dict):
        ...
        related = self.db.find_related_events('federal_register', hts_codes, pub_date, limit=5)
